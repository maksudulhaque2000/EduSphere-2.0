...
import { FaChalkboardTeacher, FaUsers, FaUserGraduate } from "react-icons/fa";
  ...
  return (
    <div className="min-h-screen bg-gradient-to-r from-teal-100 to-teal-200 flex flex-col items-center py-10">
      <motion.div className="text-center" initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ duration: 0.5 }}>
        ...
        <hr className="border-2 border-teal-800 rounded-xl" />
      </motion.div>
      ...
      <div className="w-full max-w-4xl mt-10">
        <h2 className="text-2xl font-semibold text-gray-800 text-center mb-4">Enrollment Trends</h2>
        <ResponsiveContainer width="100%" height={300}>
          <LineChart data={data}>
            <XAxis dataKey="name" />
            <YAxis />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-6 flex-1 w-full p-4">
